...
SENSOR_SCHEMA = cv.Schema(
    {
        cv.validate_id_name: cv.Schema(
            {
                cv.Optional(CONF_IGNORE, default=False): cv.boolean,
                cv.Optional(CONF_MEASUREMENT): cv.string,
                cv.Optional(CONF_TAGS, default={}): cv.Schema({cv.string: cv.string}),
            }
        )
    }
)
...
CONFIG_SCHEMA = cv.Schema(
    {
        ...
        cv.Optional(
            CONF_SEND_TIMEOUT, default="100ms"
...
def to_code(config):
    ...
    cg.add(
        var.set_tags(
            "".join(
                ",{}={}".format(tag, value) for tag, value in config[CONF_TAGS].items()
            )
        )
    )
    for sensor_id, sensor_config in config[CONF_SENSORS].items():
        if sensor_config[CONF_IGNORE] == False:
            ...
            cg.add(
                var.add_setup_callback(
                    cg.RawExpression(f"[]() -> EntityBase* {{ return {sensor_id}; }}")
                )
            )
